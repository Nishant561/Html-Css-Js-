...
form.addEventListener("click",(e)=>{
    ...
    if(id){
        ...
        let valueOfInput = parseInt(field.value);
        if(isNaN(valueOfInput)|| valueOfInput<0|| valueOfInput>text.length){
            content.innerHTML = `<p> ${text[randomNumber]}</P>`
        }else{
            let textInput = text.slice(0 , valueOfInput);
            textInput = textInput.map((items)=>{
                return(`<p>${items}</p>`)
            })
            .join("");
